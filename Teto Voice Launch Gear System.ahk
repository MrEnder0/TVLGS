...
about() {
    MsgBox Teto Voice Launch Gear System (TLGVS)`n`nDeveloper: MrEnder`nCommissioner: Heliodor`nMade in: February 2023`nVersion: 1.3.5`n`nHow to use: When application is started make sure to select your keybind for your landing gear that you have aLso set in game. When that is selected make sure to click start.`n`nHow to close: The best way to close is to press {input key} + esc + ctrl, make sure you have waited your min delay before trying to do this keybind, this will force close the app, another way to close the app is to go to your icon tray on the bottom right of your screen your icon tray should be an up arrow. On that tray you can find the app to close it right click and select "Exit".`n`nHow to configure: To configure the settings for the app click "Options" the gui should expand and reveal many options that can be changed. To confirm your settings click "Minimize" and the window should restore to normal and keep your settings for the rest of the session. If you want to save your settings and have them stay inbetween sessions edit the file named "settings.ini" this file will be in the same directory as your executable.`n`nVoiceline desync issue: Make sure you are not pressing your landing gear key while not being in the ship or it will false detect it and desync the voice lines with what is actually happening.
}
